...
function render(duration,sampleRate=22050,speed='medium'){
 const timing=Timing.schedule(duration,speed),events=[];
 ...
 function musicBeat(relative,b,period,closing){
  const at=intro+relative,gain=closing?.36:.70,bar=Math.floor(b/8),root=roots[bar%roots.length];
  events.push({time:relative,closing});
  kick(at,.40*gain*(b%8===0?1.08:1));hat(at,.11*gain);hat(at+period/2,.065*gain);if(b%2===1)snare(at,.20*gain);
  note(at,closing?110:root,Math.min(.32,period*.8),.20*gain);
  if(b%2===0)note(at,closing?440:root*2*Math.pow(2,melody[Math.floor(b/2)%melody.length]/12),Math.min(.38,period*.9),.07*gain);
 }
 for(let b=0;b<timing.rounds*8;b++)musicBeat(b*timing.beat,b,timing.beat,false);
 if(timing.closingStart-timing.bodyEnd>1e-8)note(intro+timing.bodyEnd,220,Math.min(.5,timing.closingStart-timing.bodyEnd),.035);
 for(let b=0;b<20;b++)musicBeat(timing.closingStart+b*.5,b,.5,true);
 ...
 return {data,sampleRate,intro,duration,bpm:timing.bpm,events};
}

...
vm.runInNewContext(fs.readFileSync(path.join(root,'scenes.js'),'utf8'),context);
const seq=require('../sequence.js'),music=require('../music.js'),planner=seq.createPlanner(graphs,context.window.MATH_SCENES);
assert.equal(graphs.length,1087);assert.equal(new Set(graphs.map(g=>g.id)).size,1087);
...
for(const g of graphs.filter(g=>g.kind==='log')){const points=g.path.match(/[ML](-?[\d.]+),(-?[\d.]+)/g);for(const p of points){const x=(Number(p.slice(1).split(',')[0])-400)/60;assert(x>(g.h||0))}}
...
for(const duration of [120,180])for(const accelerate of [false,true])for(const kinds of Object.values(seq.presets))for(let run=0;run<30;run++){
 ...
 const seen=new Set(frames.filter(f=>f.start<duration-10).map(f=>planner.map.get(f.graph).kind));for(const k of kinds)assert(seen.has(k),'Missing selected kind: '+k);
 const horizontal=frames.filter(f=>f.start<duration-10&&planner.map.get(f.graph).kind==='constant').reduce((s,f)=>s+f.end-f.start,0);assert(horizontal/(duration-10)<.25,'Too many horizontal lines');
...
assert.equal(new Set(graphs.filter(g=>g.kind==='linear').map(g=>g.a)).size,12);
assert(graphs.every(g=>g.mathml&&g.latex));
...
console.log('PASS: 1087 paths; log domains; tangent breaks; five exact derivatives; 360 complete sessions; fixed endings; synthesized audio and local asset references.');
